+++
title = "Notes on amateur radio"
date = 2025-07-16
updated = 2025-10-12

[taxonomies]
tags = ["radio", "ham", "antennas"]
+++

<!-- more -->

Living collection of notes on the topic of amateur radio.

## Operations

### Propagation conditions

- [dr2w.de DX](https://dr2w.de/dx-propagation/)
- [hf.dxview.org](https://hf.dxview.org/)

### Special events

- [Parks on the Air](https://docs.pota.app)
- [Summits on the Air](https://www.sota.org.uk/)
- [SOTA Atlas](https://sotl.as/about)

## Exam prep

This section covers the Polish ham radio license exams.

- [egzaminKF.pl](https://egzaminkf.pl/home.php) – exam questions, tests, topic-focused guides.
- [Anki deck – Pytanie Egzaminacyjne Klasa A](https://ankiweb.net/shared/info/1982207399) (by `SP9TEM`)

## Transceiver theory

- [Superheterodyne receivers](https://www.ire.pw.edu.pl/~wkazubski/Odbiornik1.pdf) (PL 🇵🇱) – dr inż. Wojciech Kazubski, Politechnika Warszawska

## CW (morse code)

- [LCWO](https://lcwo.net/main) — Online CW learning site.

## Digital

- [SQ9JDO](https://sq9jdo.com.pl/) — personal page of Andrzej, the Polish [DMR](https://en.wikipedia.org/wiki/Digital_mobile_radio) mastermind.
- [HamTetra](https://www.hamtetra.com/) — all about Tetra for amateur radio ops.
- [M17](https://github.com/M17-Project) — free and open-source digital radio protocol for voice and data.

## Radiosondes

- [Hot air baloon lift calculator](https://hab.sp9unb.eu/voila/render/hotair-box.ipynb) (SP9UNB).

## Antenna building

- [¼ wave GP antenna](https://m0ukd.com/calculators/quarter-wave-ground-plane-antenna-calculator/)
- [Coax cable signal loss](https://olgierd.github.io/coaxcalculator/)
- [SimNEC](https://www.ae6ty.com/smith_charts/) — RF circuit analysis
- [NanoVNA](https://nanovna.com/)
  - [nanovna-saver](https://github.com/NanoVNA-Saver/nanovna-saver) — dumping data from NanoVNA
  - [AntScope2](https://rigexpert.com/software/antscope2/#) — VNA data analysis
- Chokes
  - [Common-mode choke impedances (G3TXQ)](http://www.karinya.net/g3txq/chokes/)
